...
namespace SplatDev.Umbraco.Plugins.ShopCart.Migrations;

/// <summary>
/// Creates the ShopCart tables inside Umbraco's database.
/// </summary>
/// <remarks>
/// The plugin registered a DbContext and never created its schema — no EF migration, no
/// EnsureCreated, nothing. Every request that touched CartItems therefore failed against
/// a table that does not exist, on any database. EnsureCreated is not usable here because
/// it checks whether the *database* exists, and this context shares Umbraco's, which
/// always does; it would return false and create nothing.
///
/// The DDL comes from EF's own model so it matches the entities and the active provider,
/// and it runs inside an Umbraco migration so Umbraco records that it ran and will not
/// repeat it.
/// </remarks>
public class CreateShopCartTables : MigrationBase
{
    private readonly IServiceProvider _serviceProvider;

    public CreateShopCartTables(IMigrationContext context, IServiceProvider serviceProvider)
        : base(context) => _serviceProvider = serviceProvider;

    protected override void Migrate()
    {
        using var scope = _serviceProvider.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ShopCartDbContext>();
