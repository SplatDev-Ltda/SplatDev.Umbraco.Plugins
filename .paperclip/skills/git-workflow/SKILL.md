...
## Cloning private repos (auth)

Clone with the **plain** repo URL — the runtime's git credential helpers handle
authentication from the environment. For **Azure DevOps**
(`dev.azure.com` / `*.visualstudio.com`) the `AZURE_DEVOPS_PAT` env var is read by
a credential helper automatically:

```bash
git clone https://dev.azure.com/<org>/<project>/_git/<repo>
```

**Never** embed a token in a URL or remote (`https://oauth2:$PAT@…`, `:$PAT@…`,
`git remote set-url …$PAT…`) — that writes the secret in plaintext into
`.git/config`. Plain URLs only, for clone/fetch/push.
