...
/**
 * Choose which poll a page shows.
 *
 * The plugin's view component takes a numeric poll id, so putting a poll on a page meant
 ...
 */
@customElement("quickpoll-picker")
export class QuickPollPickerElement extends UmbElementMixin(LitElement) {
  ...
  async #load(): Promise<void> {
    try {
      const response = await this.#fetch("/umbraco/api/quickpoll/GetAll");
      if (response.ok) {
...
export default QuickPollPickerElement;

declare global {
  interface HTMLElementTagNameMap {
    "quickpoll-picker": QuickPollPickerElement;
  }
}
