...
/**
 * Choose which album a page shows.
 *
 * The plugin stores albums in its own tables and had no way at all for a page to reference
 * one — no view component, no picker. An album could be filled and never shown.
 *
 * The value stored is the numeric id, which is what the view component takes.
 */
@customElement("photogallery-picker")
export class PhotoGalleryPickerElement extends UmbElementMixin(LitElement) {
...
export default PhotoGalleryPickerElement;

declare global {
  interface HTMLElementTagNameMap {
    "photogallery-picker": PhotoGalleryPickerElement;
  }
}
