import { LitElement as b, html as l, nothing as x, css as C, property as f, state as u, customElement as E } from "@umbraco-cms/backoffice/external/lit";
...
var P = Object.defineProperty, S = Object.getOwnPropertyDescriptor, _ = (t) => {
  throw TypeError(t);
}, i = (t, e, a, n) => {
  for (var s = n > 1 ? void 0 : n ? S(e, a) : e, h = t.length - 1, c; h >= 0; h--)
    (c = t[h]) && (s = (n ? c(e, a, s) : c(s)) || s);
...
let r = class extends k(b) {
  constructor() {
    super(...arguments), v(this, o), v(this, p, F(this)), this.readonly = !1, this._rows = [], this._loaded = !1, this._failed = !1;
  }
  connectedCallback() {
  ...
  render() {
    return this._loaded ? this._failed ? l`<div class="warn">The list could not be loaded. See the browser console.</div>` : this._rows.length === 0 ? l`<div class="warn">
        There are no FAQ categories yet. Create one on the FAQs dashboard, then choose it here.
      </div>` : l`
      <uui-select
        label="FAQ category"
      ...
      ></uui-select>
      <p class="hint">Stores the FAQ category's id, which is what the page's view component takes.</p>
      ${x}
...
y = async function() {
  try {
    const t = await $(this, p).call(this, "/umbraco/api/faqs/GetCategories");
    if (t.ok) {
...
g = function(t) {
  ...
  a !== this.value && (this.value = a, this.dispatchEvent(new A()));
};
r.styles = C`
    :host { display: block; }
