...
/**
 * Choose which FAQ category a page shows.
 *
 * The plugin's view component takes a category, so putting a set of FAQs on a page meant
 * knowing its id and hardcoding it in a template. A content editor could not choose one.
 *
 * The value stored is the numeric id, which is what the view component takes.
 */
@customElement("faqs-picker")
export class FaqsPickerElement extends UmbElementMixin(LitElement) {
  ...
  async #load(): Promise<void> {
    try {
      const response = await this.#fetch("/umbraco/api/faqs/GetCategories");
      if (response.ok) {
...
export default FaqsPickerElement;

declare global {
  interface HTMLElementTagNameMap {
    "faqs-picker": FaqsPickerElement;
  }
}
