...
/// disagreed: the migration created VisitorSession and DailyVisitorCount, and the context queried
/// VisitorCounter_Session and VisitorCounter_DailyCount. The migration therefore reported success, recorded itself as done, and left
...
/// </remarks>
public class CreateVisitorCounterTablesFromModel : MigrationBase
{
    private readonly IServiceProvider _serviceProvider;

    public CreateVisitorCounterTablesFromModel(IMigrationContext context, IServiceProvider serviceProvider)
        : base(context) => _serviceProvider = serviceProvider;

    protected override void Migrate()
    {
        using var scope = _serviceProvider.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<VisitorCounterDbContext>();
