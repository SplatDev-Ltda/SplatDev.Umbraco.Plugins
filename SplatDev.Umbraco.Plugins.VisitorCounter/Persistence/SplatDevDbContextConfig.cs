...
namespace SplatDev.Umbraco.Plugins.VisitorCounter.Persistence;

/// <summary>
/// Points a plugin DbContext at the same database Umbraco is using, with the same provider.
/// </summary>
/// <remarks>
/// This used to be a bare <c>UseSqlServer(umbracoDbDSN)</c>. Umbraco supports SQLite — it is
/// what the install wizard picks by default — and handing a SQLite connection string to the
/// SQL Server provider fails with "Keyword not supported: 'cache'", surfacing as a 500 from
...
public static class SplatDevDbContextConfig
{
    ...
    /// </summary>
    /// <remarks>
    /// Umbraco resolves this itself before handing the string to its own database factory, so
    /// a plugin reading the raw configuration value gets the unexpanded token and SQLite tries
    /// to open a file with a literal pipe in its name.
    /// </remarks>
