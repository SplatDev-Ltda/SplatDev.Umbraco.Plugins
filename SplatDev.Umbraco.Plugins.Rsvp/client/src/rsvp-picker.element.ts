...
/**
 * Choose which event a page shows.
 *
 * The plugin's view component takes a numeric event id, so putting an event on a page meant
 ...
 */
@customElement("rsvp-picker")
export class RsvpPickerElement extends UmbElementMixin(LitElement) {
  ...
  async #load(): Promise<void> {
    try {
      const response = await this.#fetch("/umbraco/api/rsvp/GetEvents");
      if (response.ok) {
...
export default RsvpPickerElement;

declare global {
  interface HTMLElementTagNameMap {
    "rsvp-picker": RsvpPickerElement;
  }
}
